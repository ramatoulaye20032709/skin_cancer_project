...
IMG_SIZE        = 224
BATCH_SIZE      = 32
EPOCHS_FROZEN   = 15   # Phase 1 : backbone gelé
EPOCHS_FINETUNE = 30   # Phase 2 : fine-tuning (augmenté vs v1)
LEARNING_RATE   = 0.001
FINE_TUNE_LR    = 0.00005  # ✅ LR plus faible pour fine-tuning stable

# ✅ NOUVEAU — nombre de couches dégelées (60 au lieu de 30)
UNFREEZE_LAYERS = 60
...
print("   ✅ Augmentation + rotation/zoom")
...
# ── ✅ NOUVEAU — Oversampling des classes minoritaires ──────
print("\n[3/8] Oversampling des classes minoritaires...")
counts = train_df['label'].value_counts()
max_count = counts.max()
target_count = max_count // 2  # objectif : au moins 50% de la classe dominante
...
# ✅ AMÉLIORÉ — Augmentation avec tf pures uniquement (compatible tf.data)
def augment(img, label):
    ...
    # ✅ NOUVEAU — Rotation aléatoire via tfa ou tf.raw_ops (approche compatible)
    # Utilisation de random_crop + resize pour simuler le zoom
    crop_frac = tf.random.uniform([], 0.80, 1.0)
    crop_size = tf.cast(tf.round(crop_frac * IMG_SIZE), tf.int32)
    img = tf.image.random_crop(img, size=[crop_size, crop_size, 3])
    img = tf.image.resize(img, [IMG_SIZE, IMG_SIZE])

    # ✅ NOUVEAU — Rotation 90°/180°/270° aléatoire (opération native tf)
    k = tf.random.uniform([], minval=0, maxval=4, dtype=tf.int32)
    img = tf.image.rot90(img, k=k)

    # ✅ NOUVEAU — Cutout (masque carré aléatoire pour régularisation)
    if tf.random.uniform([]) > 0.5:
        pad = IMG_SIZE // 8  # carré de 28x28 pixels
        cx = tf.random.uniform([], pad, IMG_SIZE - pad, dtype=tf.int32)
        cy = tf.random.uniform([], pad, IMG_SIZE - pad, dtype=tf.int32)
        mask = tf.ones([IMG_SIZE, IMG_SIZE, 3], dtype=tf.float32)
        # Remplacement par la moyenne locale (simplifié : valeur 128)
        indices_x = tf.range(IMG_SIZE)
        mask_x = tf.cast((indices_x >= cx - pad) & (indices_x < cx + pad), tf.float32)
        indices_y = tf.range(IMG_SIZE)
        mask_y = tf.cast((indices_y >= cy - pad) & (indices_y < cy + pad), tf.float32)
        cutout_mask = 1.0 - tf.einsum('i,j->ij', mask_x, mask_y)[:, :, tf.newaxis]
        img = img * cutout_mask + 128.0 * (1.0 - cutout_mask)
...
def build_transfer_model():
    ...
    x = layers.BatchNormalization()(x)  # ✅ NOUVEAU — BN supplémentaire
...
# ── ✅ Métriques enrichies ─────────────────────────────────
# AUC et Recall ajoutés pour évaluation clinique
def get_metrics():
    return [
        'accuracy',
...
# ── PHASE 1 : Entraînement couches gelées ──────────────────
...
callbacks_phase1 = [
    ...
    keras.callbacks.ReduceLROnPlateau(
        monitor='val_loss',
        factor=0.5,
        patience=3,          # ✅ augmenté (2 → 3) pour plus de stabilité
        min_lr=1e-6,
        verbose=1
    ),
...
# ✅ AMÉLIORÉ — 60 couches dégelées au lieu de 30
base_model.trainable = True
for layer in base_model.layers[:-UNFREEZE_LAYERS]:
    layer.trainable = False

# Garder les BatchNorm gelées pour stabilité
...
model.compile(
    optimizer=keras.optimizers.Adam(
        learning_rate=FINE_TUNE_LR,  # ✅ 5e-5 au lieu de 1e-4
        clipnorm=1.0                  # ✅ NOUVEAU — gradient clipping
    ),
    loss='sparse_categorical_crossentropy',
    metrics=get_metrics()
)
...
for name, val in zip(metric_names, results):
    print(f"✓ Test {name:12s} : {val*100:.2f}%" if name != 'loss' else f"✓ Test {name:12s} : {val:.4f}")
...
# ✅ NOUVEAU — matrice normalisée pour mieux voir les taux par classe
...
# ── ✅ NOUVEAU — Rapport par classe (F1, recall, precision) ─
...
model_info = {
    ...
    "improvements_v2" : [
        ...
        f"Oversampling classes minoritaires (target={target_count})",
        f"{UNFREEZE_LAYERS} couches dégelées (vs 30 en v1)",
        "Fine-tuning LR réduit à 5e-5",
        "Gradient clipping (clipnorm=1.0)",
        "BatchNorm gelées pendant fine-tuning",
        "Métriques AUC + Recall + Precision",
        "Matrice confusion normalisée",
        "Graphique F1/Recall/Precision par classe"
    ]
}
...
print("  📊 matrice_confusion_v2.png     (absolue + normalisée)")
